...
template <size_t N> struct uint_from;
template <> struct uint_from<1> {
    using type = uint8_t;
};
template <> struct uint_from<2> {
    using type = uint16_t;
};
template <> struct uint_from<4> {
    using type = uint32_t;
};
template <> struct uint_from<8> {
    using type = uint64_t;
};
template <size_t N> using uint_from_t = typename uint_from<N>::type;

template <size_t N, typename T> bool ptr_alignment_is(T *ptr)
{
    return (reinterpret_cast<std::uintptr_t>(ptr) & ((1 << N) - 1)) == 0;
}

template <typename TT, typename TP> bool ptr_is_aligned_for(TP *ptr)
{
...
template <typename T, auto F> struct Handle {
    bool holding = false;
    T    actual;
    void acquire()
    {
